...
public:
	...
	BOOL		OpenUpdateProfile(LPCTSTR lpszSourceRootUrl, LPCTSTR lpszUpdateProfileXml, BOOL bUseUTF8 = TRUE);				// 업데이트 정보 파일 열기(다운로드, 파싱, 메모리 로드)
	...
	// Set callback
	void		SetReceivingStatusCallback(LPVOID lpCallbackFunc, void *pUserData);												// 다운로드 진행 상태 변화 시 호출
	void		SetReceiveCompleteCallback(LPVOID lpCallbackFunc, void *pUserData);												// 다운로드 완료 시 호출
	void		SetApplyCompleteCallback(LPVOID lpCallbackFunc, void *pUserData);												// 다운로드 된 파일을 해당 디렉토리로 복사 완료 시 호출
	...
	static BOOL		ExecuteFile(_In_ LPCTSTR lpszFilePath);																		// 파일 실행
